...
func (u *UploadSession) BulkMarkDataMapsAsUnassigned() error {
	...
	for i := 0; i < oyster_utils.MAX_NUMBER_OF_SQL_RETRY; i++ {
		err = DB.RawQuery("UPDATE data_maps SET status = ? "+
			"WHERE id IN (SELECT id FROM data_maps WHERE genesis_hash = ? AND status = ? AND message != ? AND msgStatus = ?)",
			Unassigned,
			u.GenesisHash,
			Pending,
			DataMap{}.Message,
	...
	for i := 0; i < oyster_utils.MAX_NUMBER_OF_SQL_RETRY; i++ {
		err = DB.RawQuery("UPDATE data_maps SET status = ? "+
			"WHERE id IN (SELECT id FROM data_maps WHERE genesis_hash = ? AND status = ? AND msgStatus = ?)",
			Unassigned,
			u.GenesisHash,
			Pending,
