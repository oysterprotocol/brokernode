...
func CheckPaymentToAlpha() {
	...
	for _, brokerTx := range brokerTxs {
		balance := EthWrapper.CheckPRLBalance(oyster_utils.StringToAddress(brokerTx.ETHAddrAlpha))
...
func SendGasToAlphaTransactionAddress() {
	...
	for _, brokerTx := range brokerTxs {
		...
		_, _, _, err = EthWrapper.SendETH(
			...
			oyster_utils.StringToAddress(brokerTx.ETHAddrAlpha),
			gasToSend)
...
func addressHasEnoughGas(address string) (bool, *big.Int, error) {
	gasBalance := EthWrapper.CheckETHBalance(oyster_utils.StringToAddress(address))

	gasNeeded, err := EthWrapper.CalculateGasNeeded(oyster_utils.GasLimitPRLSend)
	if err != nil {
		oyster_utils.LogIfError(err, nil)
...
func checkAndSendHalfPrlToBeta(brokerTx models.BrokerBrokerTransaction, balance *big.Int) {
	...
	betaAddr := oyster_utils.StringToAddress(brokerTx.ETHAddrBeta)
	...
	privateKey, err := oyster_utils.StringToPrivateKey(brokerTx.DecryptEthKey())
	if err != nil {
		oyster_utils.LogIfError(err, nil)
	}
	...
	callMsg, _ := EthWrapper.CreateSendPRLMessage(
		oyster_utils.StringToAddress(brokerTx.ETHAddrAlpha),
		privateKey,
		oyster_utils.StringToAddress(brokerTx.ETHAddrBeta), splitAmount)
